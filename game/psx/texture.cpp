...
/* ---- FETexture_LoadPmxAtOffset__FPciP18tTexture_ShapeInfoii  [TEXTURE.CPP:?] SLD-FLAG:NO_SLD ---- */
void FETexture_LoadPmxAtOffset(char *f,int index,tTexture_ShapeInfo *dest,int yoffset,int xoffset)

{
  char*oldptr;
  int newptr;
  u_char is_compressed;
  short tpage_or_clut;
  int newptr_or_handle;
  int shapeData;
  int shapeDataSize;
  int decompBuf;
  int uploadResult;
  ...
  if (shapeData != 0) {
    is_compressed = *(u_char *)shapeData;
    decompBuf = shapeData;
    if ((is_compressed & 0x80) != 0) {
      ...
      unpack((void *)(shapeData + 0x10),(void *)(decompBuf + 0x10));
      *(u_char *)decompBuf = *(u_char *)decompBuf & 0x7f;
      *(u_int *)decompBuf =
           (u_int)*(u_char *)decompBuf | ((shapeData + (*(int *)shapeData >> 8)) - decompBuf) * 0x100;
    }
    bpp = (u_char)*(u_int *)decompBuf & 3;
    if (bpp != 2) {
      Texture_GetClutId(bpp,&xclut,&yclut);
      dest->clutID = yclut << 6 | xclut >> 4 & 0x3fU;
    }
    frameX = *(int *)decompBuf;
    save1 = *(u_int *)(decompBuf + 4);
    save2 = *(u_int *)(decompBuf + 8);
    frameY = *(u_int *)(decompBuf + 0xc);
    save4 = *(u_int *)(decompBuf + 0x10);
    vramfxya((void *)decompBuf,
               (short)((int)(*(u_int *)(decompBuf + 0xc) << 0x14) >> 0x14) + (short)xoffset,
               (short)((int)(*(u_int *)(decompBuf + 0xc) << 4) >> 0x14) + (short)yoffset,(short)xclut
               ,yclut);
    dest->shpptr = (shapetbl *)shapeData;
    (*(u_int *)((char *)dest + 0x9)) = (u_char)*(u_int *)decompBuf;
    *(u_int *)&(*(u_int *)((char *)dest + 0xc)) = *(u_int *)&(*(u_int *)((char *)dest + 0xc)) & 0xff000000 | *(u_int *)decompBuf >> 8;
    dest->width = (short)*(u_int *)(decompBuf + 4);
    dest->height = *(short *)(decompBuf + 6);
    dest->centerx = (short)*(u_int *)(decompBuf + 8);
    dest->centery = *(short *)(decompBuf + 10);
    dest->shapex = (short)((int)(*(u_int *)(decompBuf + 0xc) << 0x14) >> 0x14);
    (*(u_short *)&uploadResult) = (short)((int)(*(u_int *)(decompBuf + 0xc) << 4) >> 0x14);
    dest->shapey = (short)uploadResult;
    uploadResult = shapedepth((void *)decompBuf);
    dest->depth = (char)uploadResult;
    dest->tpage = ((u_char)(*(u_int *)((char *)dest + 0x9)) & 3) << 7 | (short)(dest->shapey & 0x100U) >> 4 |
                  (u_short)(((u_short)dest->shapex & 0x3ff) >> 6) | (dest->shapey & 0x200U) << 2;
    tpage_or_clut = GetClut((dest->clutID & 0x3fU) << 4,dest->clutID >> 6);
    dest->clut = tpage_or_clut;
    if ((yoffset != 0) || (xoffset != 0)) {
      *(int *)decompBuf = frameX;
      *(u_int *)(decompBuf + 4) = save1;
      *(u_int *)(decompBuf + 8) = save2;
      *(int *)(decompBuf + 0xc) = frameY;
      *(u_int *)(decompBuf + 0x10) = save4;
    }
    if ((is_compressed & 0x80) != 0) {
...
/* ---- FETexture_LoadPmx__FPciP18tTexture_ShapeInfo  [TEXTURE.CPP:?] SLD-FLAG:NO_SLD ---- */
