...
for hdr in ['lib/libfns.h','lib/eaclib.h']:
    ...
    for line in open(hdr,encoding='utf-8',errors='ignore'):
        m=re.search(r'([A-Za-z_]\w*)\s*\(\s*\.\.\.\s*\)\s*;\s*/\*\s*\((.*?)\)\s*\*/', line)
        if not m: continue
        fn, params = m.group(1), m.group(2).strip()
        if params=='' or params=='void': n=0
        else:
            # count top-level commas +1; ignore '= default' (optional args still counted)
            d=0; n=1
            for ch in params:
                if ch in '([{': d+=1
                elif ch in ')]}': d-=1
                elif ch==',' and d==0: n+=1
            # mark functions whose last param has a default as min-args = n-1
        arity[fn]=n
...
# group: show only functions with >=2 documented args (dropped-arg most meaningful)
